...
(function () {
	...
	var supports$1 = 'querySelector' in document && 'addEventListener' in root && 'classList' in document.createElement('_'); // Feature test
	var settings, app;
	...
	astro.toggleNav = function ( toggle, navID, options, event ) {
		...
		var settings = extend$1( settings || defaults$1, options || {} );  // Merge user options with defaults
	...
	astro.init = function ( options ) {
		...
		// Selectors and variables
		settings = extend$1( defaults$1, options || {} ); // Merge user options with defaults
	...
	window.astro = astro;
	window.BetterDocs = BetterDocs;
	window.docsNavCollapse = docsNavCollapse;
	window.dropdown = addDropdown;
	window.fluidvids = fluidvids;
	window.latestBlogPosts = latestBlogPosts;
